...
def format_trivy_result(scan_data: dict | list) -> str:
    ...
    if critical_count > 0 or high_count > 0:
        ...
        if critical_count > 0:
            output_lines.append(f"   • Срочно обновите пакеты с {critical_count} CRITICAL уязвимостями")
        if high_count > 0:
            output_lines.append(f"   • Запланируйте обновление для {high_count} HIGH уязвимостей")
